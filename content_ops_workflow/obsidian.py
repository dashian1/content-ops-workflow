...
def plugin_status() -> dict[str, Any]:
    if not rest_enabled():
        return {
            "ok": False,
            "mode": "file",
            "message": "未配置 OBSIDIAN_API_KEY，当前使用文件夹写入模式。",
    ...
    return {
        ...
        "mode": "plugin" if response.status_code < 400 else "file",
...
def write_note(vault_path: str, body: str) -> ObsidianWriteResult:
    ...
    with open(local_path, "w", encoding="utf-8") as f:
        f.write(body)
    return ObsidianWriteResult(True, "file", vault_path, local_path, uri, error=error)
